        ...
        private void FormStorage_Load(object sender, EventArgs e)
        {
            if (id.HasValue)
            {
                try
                {
                    ...
                    var storageAutoParts = storageList[0].StorageAutoParts;
                    for (int i = 0; i < storageList.Count; ++i)
                    {
                        if (storageList[i].Id == id)
                        {
                            storageAutoParts = storageList[i].StorageAutoParts;
                        }
                    }
                    if (storageAutoParts != null)
                    {
                        dataGridView.DataSource = storageAutoParts;
